...
const toptenmentorsbasedonrequest = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No mentors found with requests." });
    }
...
const getMentorEndorsements = async (req, res) => {
  ...
  try {
    ...
    const [results] = await db.query(mentorEndorsementsQuery, [
      firstName,
      lastName,
      phonenumber,
    ]);
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No endorsements found for the specified mentor." });
    }
...
const getAllRequestByHobbyistInfo = async (req, res) => {
  ...
  try {
    ...
    const [results] = await db.query(hobbyistRequestsQuery, [
      phonenumber,
      fname,
      lname,
    ]);
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No requests found for the specified hobbyist." });
    }
  ...
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: "No posts or comments found for the specified hobbyist.",
      });
    }
...
const getPostsBySkill = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: "No posts or comments found for the specified skill.",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching posts by skill." });
  }
};
...
const getPostsByMentorLocation = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: "No posts or comments found for the specified location.",
      });
    }
    ...
    console.error(error);
    res.status(500).json({
      message: "An error occurred while fetching posts by mentor location.",
    });
  }
};
...
const getPostsAndCommentsByMentorSchooling = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: "No posts or comments found for the specified schooling.",
      });
    }
    ...
    res.status(500).json({
      message:
        "An error occurred while fetching posts and comments by mentor schooling.",
    });
  }
};
...
const getPostsBySkillLocationSchooling = async (req, res) => {
  try {
    ...
    const [results] = await db.query(postsBySkillLocationSchoolingQuery, [
      skillname,
      location,
      schooling,
    ]);
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: "No posts or comments found for the specified criteria.",
      });
    }
    ...
    res.status(500).json({
      message:
        "An error occurred while fetching posts by skill, location, and schooling.",
    });
  }
};
...
const getHobbyistComments = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No comments found for any hobbyist." });
    }
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching hobbyist comments." });
  }
};
...
const getTopEventPosts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching top events." });
  }
};
...
const getEventsBySkill = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No events found for the specified skill." });
    }
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching events by skill." });
  }
};
...
const getEventsByLocation = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No events found for the specified location." });
    }
...
const getEventsByMentorSchooling = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: "No events found for the specified mentor schooling.",
      });
    }
    ...
    console.error(error);
    res.status(500).json({
      message: "An error occurred while fetching events by mentor schooling.",
    });
  }
};
...
const getEventsBySkillLocationSchooling = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message:
          "No events found for the specified skill, location, and mentor schooling.",
      });
    }
    ...
    res.status(500).json({
      message:
        "An error occurred while fetching events by skill, location, and mentor schooling.",
    });
  }
};
...
const getEventsByHobbyist = async (req, res) => {
  try {
    ...
    if (hobbyistResults.length === 0) {
      return res
        .status(404)
        .json({ message: "Hobbyist not found with the provided details." });
    }
    ...
    if (eventsResults.length === 0) {
      return res
        .status(404)
        .json({ message: "No events found for the specified hobbyist." });
    }
    ...
    console.error(error);
    res.status(500).json({
      message: "An error occurred while fetching events by hobbyist.",
    });
  }
};
