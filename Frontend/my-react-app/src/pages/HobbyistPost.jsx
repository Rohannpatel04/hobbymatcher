...
export default function HobbyistPost() {
  ...
  const [getpostsandcommentsbyhobbyist, setGetPostsAndCommentsByHobbyist] =
    useState({
      firstname: "",
      lastname: "",
      phonenumber: "",
  ...
  const [
    getpostsandcommentsbyhobbyistresult,
    setGetPostsAndCommentsByHobbyistResult,
  ] = useState("");
  ...
  const [getpostsbymentorschoolingresult, setGetPostsByMentorSchoolingResult] =
    useState("");

  const [
    getpostsbyskilllocationschooling,
    setGetPostsBySkillLocationSchooling,
  ] = useState({
    skillname: "",
    location: "",
    schooling: "",
  ...
  const [
    getpostsbyskilllocationschoolingresult,
    setGetPostsBySkillLocationSchoolingResult,
  ] = useState("");
    ...
    if (formType === "post") {
      ...
      try {
        const response = await fetch(
          "http://localhost:3001/hobbyist/createcomment",
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    if (
      (getpostsbyskilllocationschooling.skillname,
      getpostsbyskilllocationschooling.location,
      getpostsbyskilllocationschooling.schooling)
    ) {
      const fetchMentorData = async () => {
        try {
          const response = await fetch(
            `http://localhost:3001/hobbyist/getpostsbyskilllocationschooling/${getpostsbyskilllocationschooling.skillname}/${getpostsbyskilllocationschooling.location}/${getpostsbyskilllocationschooling.schooling}`
          );
          if (!response.ok) {
  ...
  useEffect(() => {
    if (getpostsbymentorschooling.schooling) {
      const fetchMentorData = async () => {
        try {
          const response = await fetch(
            `http://localhost:3001/hobbyist/getpostsandcommentsbymentorschooling/${getpostsbymentorschooling.schooling}`
          );
          if (!response.ok) {
            throw new Error("Hobbyists not found");
          }
  ...
  useEffect(() => {
    if (getpostsbylocation.location) {
      const fetchMentorData = async () => {
        try {
          const response = await fetch(
            `http://localhost:3001/hobbyist/getpostsbymentorlocation/${getpostsbylocation.location}`
          );
          if (!response.ok) {
            throw new Error("Hobbyists not found");
          }
  ...
  useEffect(() => {
    if (getpostsbyskill.skillname) {
      const fetchMentorData = async () => {
        try {
          const response = await fetch(
            `http://localhost:3001/hobbyist/getpostsbyskill/${getpostsbyskill.skillname}`
          );
          if (!response.ok) {
            throw new Error("Hobbyists not found");
          }
  ...
  useEffect(() => {
    if (
      ...
      const fetchMentorData = async () => {
        try {
          const response = await fetch(
            `http://localhost:3001/hobbyist/getpostsandcommentsbyhobbyist/${getpostsandcommentsbyhobbyist.firstname}/${getpostsandcommentsbyhobbyist.lastname}/${getpostsandcommentsbyhobbyist.phonenumber}`
          );
          if (!response.ok) {
            throw new Error("Hobbyists not found");
          }
  ...
  useEffect(() => {
    const fetchAllPosts = async () => {
      try {
        const response = await fetch(
          "http://localhost:3001/hobbyist/getAllPosts"
        );
        if (!response.ok) {
          throw new Error("Hobbyists not found");
        }
  ...
  return (
    <div>
      ...
      <div>
        ...
        <form>
          ...
          <input
            ...
            onChange={(e) =>
              setGetPostsAndCommentsByHobbyist({
                ...getpostsandcommentsbyhobbyist,
        ...
        {getpostsandcommentsbyhobbyistresult &&
        Array.isArray(getpostsandcommentsbyhobbyistresult.data) &&
        getpostsandcommentsbyhobbyistresult.data.length > 0 ? (
          <table>
            <thead>
              <tr>
                <th>Post Content</th>
      ...
      <div>
        ...
        {getpostsbyskillresult &&
        Array.isArray(getpostsbyskillresult.data) &&
        getpostsbyskillresult.data.length > 0 ? (
          <table>
            <thead>
              <tr>
                <th>Post Content</th>
      ...
      <div>
        ...
        {getpostsbylocationresult &&
        Array.isArray(getpostsbylocationresult.data) &&
        getpostsbylocationresult.data.length > 0 ? (
          <table>
            <thead>
              <tr>
                <th>Post Content</th>
      ...
      <div>
        ...
        <form>
          ...
          <input
            ...
            onChange={(e) =>
              setGetPostsByMentorSchooling({
                ...getpostsbymentorschooling,
                schooling: e.target.value,
              })
            }
          />
        </form>
        {getpostsbymentorschoolingresult &&
        Array.isArray(getpostsbymentorschoolingresult.data) &&
        getpostsbymentorschoolingresult.data.length > 0 ? (
          <table>
            <thead>
              <tr>
                <th>Post Content</th>
      ...
      <div>
        ...
        {getpostsbyskilllocationschoolingresult &&
        Array.isArray(getpostsbyskilllocationschoolingresult.data) &&
        getpostsbyskilllocationschoolingresult.data.length > 0 ? (
          <table>
            <thead>
              <tr>
                <th>Post Content</th>
            ...
            <tbody>
              {getpostsbyskilllocationschoolingresult.data.map(
                (post, index) => (
                  <tr key={index}>
                    <td>{post.PostContent}</td>
                    <td>{post.MentorFullName}</td>
        ...
        <form onSubmit={(e) => handleSubmit(e, "post")}>
          ...
          <input
            ...
            onChange={(e) =>
              setCreateComment({
                ...createcomment,
                hobbyistLastName: e.target.value,
              })
            }
            className="form-control"
            required
          ...
          <input
            ...
            onChange={(e) =>
              setCreateComment({
                ...createcomment,
                hobbyistPhoneNumber: e.target.value,
              })
            }
            className="form-control"
            required
